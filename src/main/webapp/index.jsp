...
<html>
	...
	<body>
		...
		<!--  On affiche ici une message d'erreur -->
		<div><h4>${message}</h4></div>
		<div class="table">
			<div class="thead"><div class="td">Code</div><div class="td">Taux</div><div class="td">Action</div></div>
			<div class="tbody">
			<c:forEach var="enregistrement" items="${codes}">
				<form class="tr" method="get">
					<div class="td"><input type="text" name="code" value="${enregistrement.codeDiscount}" readonly/></div>
					<div class="td"><input name="taux" type="number" step="0.01" min="0.0" max="99.99" size="5" value="${enregistrement.taux}"/></div>
					<div class="td"><input type="submit" name="action" value="SUPPRIMER"/></div>
				</form>	  		    
			</c:forEach>  
			</div>
		</div>
